...
export default function HomePage() {
  ...
  return (
    ...
    <div className="min-h-screen bg-white">
      ...
      {/* --- ORIGINAL TRUST BADGES (border-ivory-200 به border-gray-200 تغییر کرد) --- */}
      <section className="border-b border-gray-200 bg-white py-8">
        <div className="container-luxury">
          <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
            {[
              { icon: Truck, title: 'ارسال رایگان', desc: 'برای سفارش‌های بالای ۵ میلیون' },
              { icon: ShieldCheck, title: 'ضمانت اصالت', desc: 'تضمین کیفیت و اصالت کالا' },
              { icon: RefreshCw, title: 'بازگشت ۷ روزه', desc: 'امکان بازگشت بدون قید و شرط' },
              { icon: Sparkles, title: 'کیفیت لوکس', desc: 'مواد درجه یک و ساخت ظریف' },
            ].map((item, i) => (
      ...
      {/* --- ORIGINAL PRODUCT SLIDER (Unchanged) --- */}
      <section className="py-16 lg:py-24">
        <div className="container-luxury">
          <div className="mb-10 flex items-end justify-between">
            <div>
              <span className="mb-2 block text-sm text-gold-500">محصولات منتخب</span>
              <h2 className="text-3xl font-bold text-navy-900 lg:text-4xl">کالکشن ویژه</h2>
            </div>
            <Link to="/products" className="hidden items-center gap-2 text-sm text-navy-700 transition-colors hover:text-navy-900 sm:flex">
              مشاهده همه
              <ArrowLeft className="h-4 w-4" />
            </Link>
          </div>
          <ProductSlider products={collectionProducts.length ? collectionProducts : featured} collectionItems={collectionItems} />
        </div>
      </section>

      {/* --- ORIGINAL CATEGORIES (bg-ivory-200/50 به bg-white تغییر کرد) --- */}
      <section className="bg-white py-16 lg:py-24">
        <div className="container-luxury">
          <div className="mb-10 text-center">
            <span className="mb-2 block text-sm text-gold-500">دسته‌بندی‌ها</span>
            <h2 className="text-3xl font-bold text-navy-900 lg:text-4xl">کالکشن‌های اویتا</h2>
            <p className="mt-3 text-sm text-gray-500">اکسسوری‌هایی برای هر استایل و هر لحظه</p>
          </div>
      ...
      {/* --- ORIGINAL NEW PRODUCTS (Unchanged) --- */}
      <section className="py-16 lg:py-24">
        <div className="container-luxury">
          <div className="mb-10 flex items-end justify-between">
            <div>
              <span className="mb-2 block text-sm text-gold-500">تازه‌های اویتا</span>
              <h2 className="text-3xl font-bold text-navy-900 lg:text-4xl">جدیدترین محصولات</h2>
            </div>
            <Link to="/products" className="hidden items-center gap-2 text-sm text-navy-700 transition-colors hover:text-navy-900 sm:flex">
              مشاهده همه
              <ArrowLeft className="h-4 w-4" />
      ...
      {/* --- ORIGINAL NEWSLETTER (bg-ivory-200/50 به bg-white تغییر کرد) --- */}
      <section className="bg-white py-16">
        <div className="container-luxury">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="mb-3 text-2xl font-bold text-navy-900 lg:text-3xl">عضویت در خبرنامه اویتا</h2>
